...
#     self.create_subscription(Float64, '/teleop/rotate', self.rotateAngle)
...
class BaseControl(Node):
    def __init__(self):
        ...
        # Publisher
        self.twist_pub = self.create_publisher(Twist, '/cmd_vel', queue_size = 1)
        # Subscriber
        self.create_subscription(Odometry, '/odom', self.odomCB)
        # Value
        self.twist_value = Twist()
    ...
    def odomCB(self, receive_msg):
        ...
        #print(self.quaternion)
        self.current_euler = tf2_geometry_msgs.transformations.euler_from_quaternion(self.quaternion)
        #print(self.current_euler)
        self.current_deg = math.degrees(self.current_euler[2])
        if self.current_deg < 0.0:
            sub_deg = 180 - abs(self.current_deg)
    ...
    def publishAnglarZ(self, max_speed, precision, time_out):
        ...
        start_time = time.time()
        start_plot = time.time()
        while self.roundDown(self.judg_deg, precision) != self.roundDown(self.current_deg, precision) and not rclpy.is_shutdown():
            plot_time = time.time() - start_plot
            delta_time = time.time() - start_time
            # 0度をまたがないとき
            if self.sub_target_deg == 0.0:
